...
export default (app: Probot) => {
  ...
  app.on(["pull_request.opened", "pull_request.synchronize"], async (context) => {
    ...
    try {
      ...
      const hasMergeConflicts = !pr.mergeable && pr.mergeable_state === "dirty";
      
      // If GitHub hasn't computed the mergeable status yet (it's null), we need to fetch it again
      if (pr.mergeable === null) {
        app.log.info(`Mergeable status not yet computed for PR #${pr.number}, waiting...`);
        ...
        if (updatedPr.mergeable === false && updatedPr.mergeable_state === "dirty") {
  ...
  async function processWithMergeConflicts(context: any, prNumber: number) {
    ...
    try {
      ...
      app.log.info(`GitSynth API response for PR #${prNumber}: ${JSON.stringify(result)}`);
      ...
      await context.octokit.issues.createComment({
        ...
        repo: repo.repo,
        issue_number: prNumber,
        body: "✅ GitSynth has attempted to resolve the merge conflicts. Please check the PR!"
      });
